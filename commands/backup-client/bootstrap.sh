...
function run {
    ...
    if [[ ! -f /tmp/mnt/root/.swap/swapfile ]]; then
        ...
        if ! runCmd dd if=/dev/zero of=/tmp/mnt/root/.swap/swapfile bs=1M count=2048 status=progress; then logError "Failed to fallocate 2G Swap-File at /tmp/mnt/root/.swap/swapfile"; exit 1; fi;
