...
# /ssh_restore.sh [-q|--quiet] [-n|--name <clienthostname>] [-s|--server ssh://user@host:port] <command>

# Command drive
# /ssh_restore.sh [-q|--quiet] [-n|--name <clienthostname>] [-s|--server ssh://user@host:port] drive [--target <targetdrive>] [-s|--snapshot <snapshot>] [-nc|--nocrypt] [--test]

# Command snapshot
# /ssh_restore.sh [-q|--quiet] [-n|--name <clienthostname>] [-s|--server ssh://user@host:port] snapshot [--target <targetvolume>] [-s|--snapshot <snapshot>] [--test] <volume>
...
function manager() {
    ...
    while [[ "$#" -gt 0 ]]; do
        case $1 in
            ...
            printManagerHelp; exit 0;;
            -*)
                logError "Unknown Argument: $1";
            printManagerHelp; exit 1;;
            *)
                ENTRY_COMMAND="${1}";
                shift;
            break;;
    ...
    if isTrue ${DEBUG:-}; then
        logDebug "Loading all functions";
        for f in ${SCRIPT_SOURCE%/*}/includes/functions/*.sh; do source $f; done;
    fi;
    ...
    if [[ "${ENTRY_COMMAND,,}" == "help" ]]; then
        printManagerHelp;
        exit 0;
    fi;
    
    if containsIllegalCharacter "${ENTRY_COMMAND}"; then logError "command contains illegal characters"; exit 1; fi;
    if [[ ! -f "${SCRIPT_SOURCE%/*}/includes/commands/${ENTRY_COMMAND,,}.sh" ]]; then
        logError "Unknown Command: ${ENTRY_COMMAND}";
        exit 1;
    fi;
    
    stopIfNotElevated;
    
    source "${SCRIPT_SOURCE%/*}/includes/commands/${ENTRY_COMMAND,,}.sh";
}
