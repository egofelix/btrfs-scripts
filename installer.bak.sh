...
# Setup IP?
targetIp=$(dig +short ${TARGET_HOSTNAME})
if [[ "${targetIp}" == "192.168.*" ]]; then
	targetIface=$(ip addr list | grep -e '192\.168\.[0-9]*\.[0-9]*\/24' | grep -oe 'ens[0-9]*')
	otherIfaces=$(comm -23 <(ip addr list | grep -oe 'ens[0-9]*' | sort | uniq | grep "\S") <(ip addr list | grep -e '192\.168\.[0-9]*\.[0-9]*\/24' | grep -oe 'ens[0-9]*' | grep "\S"))
fi;
