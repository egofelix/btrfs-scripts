...
if isTrue "${CRYPTED}"; then
	...
	if [[ ( $(getSystemType) = "ARMHF" ) ]]; then
		if ! runCmd cryptsetup --batch-mode luksFormat --type luks2 --cipher aes-cbc-essiv --key-size 256 --hash sha256 --pbkdf argon2i --sector-size 512 --use-urandom -d /tmp/crypto.key ${PART_SYSTEM}; then echo "Failed to cryptformat SYSTEM-Partiton"; exit; fi;
	else
